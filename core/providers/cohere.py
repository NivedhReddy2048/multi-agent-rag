...
class CohereProvider(BaseProvider):
    """Cohere LLM Provider integration wrapper."""

    def __init__(self):
        super().__init__(name="cohere", category=ProviderCategory.LLM, is_optional=True)
        self.api_key = Config.COHERE_API_KEY
        self.primary_model = Config.COHERE_MODEL

    def initialize(self) -> bool:
        if not self.api_key:
            self.last_error = "COHERE_API_KEY is missing."
