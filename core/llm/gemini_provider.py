...
logger = get_logger("core.llm.gemini_provider")
...
class GeminiProvider(BaseLLMProvider):
    ...
    def generate(
        ...
        logger.info(f"ENTER GEMINI provider generation | Model: {model_name}")
        try:
            ...
            logger.info(f"EXIT GEMINI provider generation | Model: {model_name} | Success")
            return content, model_name, prompt_tokens, completion_tokens
        except Exception as e:
            logger.warning(f"EXIT GEMINI provider generation | Model: {model_name} | FAILED: {e}")
            raise e
