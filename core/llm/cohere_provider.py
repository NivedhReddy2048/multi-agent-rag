"""Cohere AI LLM Provider Implementation."""
...
logger = get_logger("core.llm.cohere_provider")
...
class CohereProvider(BaseLLMProvider):
    ...
    def __init__(self, primary_model: str, fallback_model: str, api_key: str):
        super().__init__("cohere", primary_model, fallback_model, api_key)
    ...
    def generate(
        ...
        logger.info(f"ENTER COHERE provider generation | Model: {model_name}")
        try:
            ...
            logger.info(f"EXIT COHERE provider generation | Model: {model_name} | Success")
            return content, model_name, prompt_tokens, completion_tokens
        except Exception as e:
            logger.warning(f"EXIT COHERE provider generation | Model: {model_name} | FAILED: {e}")
            raise e
